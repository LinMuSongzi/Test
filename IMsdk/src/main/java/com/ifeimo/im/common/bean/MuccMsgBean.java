...
//        throw new Exception("此消息是群聊");
    ...
    public  static MuccMsgBean createLineByCursor(Cursor cursor){
        ...
        muccMsgBean.setId(cursor.getInt(cursor.getColumnIndex(MuccProvider.DB_ID)));
        muccMsgBean.setSendType(cursor.getInt(cursor.getColumnIndex(MuccProvider.DB_SEND_TYPE)));
        muccMsgBean.setMemberId(cursor.getString(cursor.getColumnIndex(MuccProvider.DB_MEMBER_ID)));
        muccMsgBean.setCreateTime(cursor.getString(cursor.getColumnIndex(MuccProvider.DB_CREATE_TIME)));
        muccMsgBean.setRooomID(cursor.getString(cursor.getColumnIndex(MuccProvider.DB_ROOM_ID)));
        muccMsgBean.setContent(cursor.getString(cursor.getColumnIndex(MuccProvider.DB_CONTENT)));
        muccMsgBean.setMemberNickName(cursor.getString(cursor.getColumnIndex(MuccProvider.DB_MEMBER_NICKNAME)));
        muccMsgBean.setMemberAvatarUrl(cursor.getString(cursor.getColumnIndex(MuccProvider.DB_MEMBER_AVATARURL)));
        return muccMsgBean;
    ...
    public static MuccMsgBean buildMuccBean(org.jivesoftware.smack.packet.Message message) {
        ...
        if (message.getBody() != null && !message.getBody().equals("")) {
            ...
            muccMsgBean.setSendType(BaseProvider.SEND_FINISH);
